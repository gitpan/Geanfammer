...
# Usage: sw_exp_search.pl <query database> <target database> (both in fasta format) <ssearch version path>
# Function: to allow a search of a whole family to be run automatically using fasta.

$|=1;

$usage="sw_exp_search.pl <query database> <target database> <ssearch version>";
...
&sw_search ($qdb_main, $tdb_main, $fastaver_main);
