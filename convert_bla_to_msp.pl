...
# Usage     : convert_bla_to_msp.pl *.pbla or *.bpla.gz
...
my ($i, @files);
$evalue_thresh=0.01;
$accumulative_hits_eval_thresh=$evalue_thresh/10;
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~````
# This is to handle multiple xxxx.bla or xxx.pbla files, but does not work,
#  So, make a wrapper program or use my 'run_bla_to_msp.pl' file
#__________________________________________________________________
for($i=0; $i< @files; $i++){
    my ($j, @keys, $sort_by_evalue, %hash_out_final);
    my $file=$files[$i];
    my $base=${&get_base_names(\$file)};
    my $out_msp="$base\.msp";
    my $Accumulate_matches='A';

    print "\n# (i)  \$i=$i $0: $file output   msp  file  will  be    \"$out_msp\"";
    print "\n# (i) \$Accumulate_matches is set to $Accumulate_matches" if $Accumulate_matches;
    if(-s $out_msp and !$over_write){
...
sub put_msp_lines_to_hash_from_bla{
     ...
     if($score=~/\S/ and $evalue=~/\S/ and $match_string_start=~/\S/ and $evalue_thresh > $evalue){
         ...
         if($hash_out{$sorted_name}=~/^\S+ +(\S+) +/){
             if($1 >= $evalue){  print "                    (1) put_msp_lines_to_hash_from_bla: $1 >= $evalue WRITING to hash. 1\n";
                 $hash_out{$sorted_name}=$msp_line;
             }else{  print "                    put_msp_lines_to_hash_from_bla: $1 < $evalue_ori NO write to hash\n";  }
         }else{
