...
# Function  : to single link all the genes in msp files in a given directory at a given e-value threshhold.
# Example   :
# Keywords  :
# Options   :
	...
	for ($j=0; $j<@mspcont; $j++){
		if ($mspcont[$j]=~/^\d+\s+(\S+)\s+\S+\s+\d+\s+\d+\s+(\S+)\s+\d+\s+\d+\s+(\S+)/){
			...
			if ( $hash{"$gene_1"} gt  $hash{"$gene_2"}){
			    $hash{"$gene_1"}=$hash{"$gene_2"};
			    next;
			}
			if ( $hash{"$gene_2"} gt  $hash{"$gene_1"}){
			    $hash{"$gene_2"}=$hash{"$gene_1"};
			    next;
			}
			next;
		}else{next;}
...
sub print_clusfile_from_hash {
    ...
    $single_linkage_cluster="single_linkage_cluster";
...
for ($i=0; $i<@clus_sizes; $i++){
    ...
    for ($j=0; $j<@subclus; $j++){
	my $clus=$subclus[$j];
	print SING "Cluster number $clus\n";
	for ($k=0; $k<@{"$clus"}; $k++){
	    $gene=${"$clus"}[$k];
	    print SING " 1 1 $gene\n";
	    next
